...
class Category
{
    ...
    public function getAllCategory($page = 1, $limit = 10, $keyword = '', $active = null)
    {
        $offset = ($page - 1) * $limit;
        $search = '';
     ...
     */
    public function getOneCategory($id, $active = 1)
    {
        if ($active !== null) {
        ...
        $query = "SELECT * FROM `categories` WHERE `id` = :id $search LIMIT 1";
        $stmt = $this->connection->prepare($query);
